...
const createAiGateway = ({ providerFactory = createProvider, sleepFn = sleep, now = () => Date.now() } = {}) => ({
  async run({ task, input, userId, requestId = crypto.randomUUID(), validate, messages, schema, policy: policyOverride }) {
    ...
    for (let index = 0; index < policy.maxAttempts; index += 1) {
      ...
      const provider = providerFactory(tier);
      const startedAt = now();
      const controller = new AbortController();
      const timer = setTimeout(() => controller.abort(), policy.timeoutMs);
      try {
        const result = await provider.completeStructured({ task, input, userId, requestId, messages, schema, signal: controller.signal });
        if (validate) {
          const validation = validate(result);
          if (!validation.ok) throw new AiError(validation.message || 'AI 结果不符合预期格式', 'AI_INVALID_OUTPUT', { retryable: true });
        }
        attempts.push({ attempt, tier, provider: provider.name, model: provider.model, status: 'succeeded', latencyMs: now() - startedAt });
        return { data: result, requestId, attempts };
      } catch (error) {
        const normalized = normalizeProviderError(error);
        lastError = normalized;
        attempts.push({ attempt, tier, provider: provider.name, model: provider.model, status: 'failed', code: normalized.code, retryable: normalized.retryable, latencyMs: now() - startedAt });
